...
def merge_armatures(target_armature, extra_armature, orphan_parent_map):
    ...
    def merge_move_objects(target_armature, extra_armature):
        ...
        for obj in bpy.data.objects:
            if obj.parent == extra_armature:
                ...
                obj.matrix_world = (
                    extra_armature.matrix_world.inverted() @ obj.matrix_world
                )
                ...
                print(
                    f"Moving from  {extra_armature.name} -> {target_armature.name}:  {obj.name}"
                )
    ...
    def get_bone_parenting(target_armature, extra_armature):
        ...
        }  # Set of bone names in target armature
        original_parents = {}
        ...
        for orphan, new_parent_bone in orphan_parent_map.items():
            if orphan in edit_bones:
                ...
                if new_parent_bone in edit_bones:
                    ...
                    print(
                        f"Warning: New parent bone {new_parent_bone} not found for {orphan}."
                    )
            else:
                print(
                    f"Warning: Orphan bone {orphan} not found in the merged armature."
                )
