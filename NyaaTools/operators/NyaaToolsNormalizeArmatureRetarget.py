...
from ..avatar.get_armature_avatar_name import get_armature_avatar_name
...
class NyaaToolsNormalizeArmatureRetarget(bpy.types.Operator):
	...
	new_animation_name: StringProperty(
		name="Animation Name",
		description="Name for the new retargeted animation"
	)
...
def resolve_target_and_extra_armatures():
	...
	if len(bpy.context.selected_objects) < 2:
		raise Exception("Please select exactly two armatures, with the main armature active.")

	# Check 1st and 2nd slot of selected objects
	is_1st_avatar = get_armature_avatar_name(bpy.context.selected_objects[0])
	is_2nd_avatar = get_armature_avatar_name(bpy.context.selected_objects[1])
	...
	if is_1st_avatar and not is_2nd_avatar:
		...
		if target_armature == bpy.context.selected_objects[0]:
			...
			raise Exception("Please active select one of the armatures to be the base (ctrl + click)")
