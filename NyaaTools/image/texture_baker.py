...
def bake_dtp_texture(
    ...
    principled_tree = tree_stack[-1]  # Last tree in stack is where principled BSDF is located
    ...
    try:
        if bake_type == 'NORMAL':
            ...
            if max_resolution:
                ...
                final_resolution = (min(final_resolution[0], max_width), min(final_resolution[1], max_height))
            
            # Bake normal
            result = _bake_normal_output(
                dtp_format, material, shader_state, material.node_tree, principled_tree,
                normal_output_node, final_resolution, debug_print
            )
            
        else: # EMIT
            # Stage emission nodes and determine resolution
            rgb_output_node, alpha_output_node, final_resolution = _stage_emission_nodes(
                tree_stack, channels, shader_state, principled_bsdf, principled_tree, resolution, max_resolution, debug_print
            )
            ...
            result = _bake_emission_output(
                dtp_format, material, shader_state, material.node_tree,
                rgb_output_node, alpha_output_node, final_resolution, debug_print
            )
...
def _stage_emission_nodes(
        ...
        if isinstance(socket_mapping, tuple):
            ...
            if socket_name not in separate_nodes:
                ...
                if socket and socket.is_linked:
                    ...
                    separate_rgb = shader_state.add_node(principled_tree, 'ShaderNodeSeparateColor')
                    separate_rgb.mode = 'RGB'
                    from_socket = socket.links[0].from_socket
                    shader_state.connect_sockets(
                        principled_tree, 
                        from_socket.node, from_socket.name, 
                        separate_rgb, 'Color', 
    ...
    shader_state.connect_sockets(principled_tree, combine_rgb_node, 'Color', rgb_emission, 'Color', "combine RGB to emission")
    ...
    if resolution:
        ...
        if not has_rgb_connected:
            ...
            rgb_resolution = detect_best_resolution(rgb_emission.inputs['Color'], tree_stack)
        ...
        if alpha_emission:
            if not alpha_emission.inputs['Color'].is_linked:
                ...
                alpha_resolution = detect_best_resolution(alpha_emission.inputs['Color'], tree_stack)
...
def _bake_emission_output(
    ...
    final_image = _pack_rgba_images(
        material, dtp_format, rgb_image, alpha_image, 
        resolution, debug_print
    )
...
def _bake_normal_output(
    ...
    if hasattr(normal_output_node, 'is_linked') and normal_output_node.is_linked:
        ...
        shader_state.connect_sockets(principled_tree, from_socket.node, from_socket.name, temp_principled, 'Normal', "normal map to temp principled")
    
    # Connect temp Principled BSDF to Material Output (both in principled_tree)
    shader_state.connect_sockets(principled_tree, temp_principled, 'BSDF', temp_material_output, 'Surface', "temp principled to material output")
        ...
        if isinstance(socket_mapping, tuple):
            ...
            if socket and socket.is_linked and not is_unused_emission:
                ...
                if scalar:
                    ...
                    debug_print(f"ERROR: Scalar channel {channel} should not be color-based")
                    continue
                else:
                    # For RGB, connect single channel
                    target_node.inputs[input_name].default_value = separate_rgb.outputs[channel_names[channel_index]].default_value
                ...
                if is_unused_emission:
                    ...
                    if scalar:
                        ...
                        target_node.inputs[input_name].default_value = socket.default_value[channel_index]
                else:
                    # It's already a single float value
                    if scalar:
                        # For scalar, convert to RGB color
                        target_node.inputs[input_name].default_value = (socket.default_value, socket.default_value, socket.default_value, 1.0)
                    else:
            ...
            if socket and socket.is_linked:
                ...
                shader_state.connect_sockets(principled_tree, from_socket.node, from_socket.name, target_node, input_name, f"channel {channel} to {input_name}")
            else:
                # Not socketed, copy value directly
                if scalar:
...
def _pack_rgba_images(
    ...
    if rgb_image.size[0] != alpha_image.size[0] or rgb_image.size[1] != alpha_image.size[1]:
        raise ValueError(f"RGB and Alpha images have different sizes: {rgb_image.size[0]}x{rgb_image.size[1]} vs {alpha_image.size[0]}x{alpha_image.size[1]}")
